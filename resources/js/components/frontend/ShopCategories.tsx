...
export default function ShopCategories() {
    ...
    return (
        <div className="w-full overflow-hidden rounded-2xl bg-gradient-to-b from-amber-50/70 to-amber-50/30 py-8
                px-4 md:px-8 relative border-y border-amber-100/50">
            <div className="max-w-7xl mx-auto">
                ...
                <div
                    className={`flex items-center justify-between mb-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"
                        }`}
                >
                    <div>
                    ...
                    >
                        <div className="w-full grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-8 gap-4 md:gap-6">
                            {visibleCategories().map((category, idx) => (
                                ...
                                >
                                    <div
                                        className={`w-full aspect-square rounded-full overflow-hidden ${category.color
                                            } p-1.5 ${hoveredCategory === category.id
                                                ? "shadow-md ring-2 ring-amber-300 ring-opacity-50"
                                                : "shadow-sm"
                                            } transition-all duration-300 transform ${hoveredCategory === category.id ? "scale-105" : ""
                                            }`}
                                    >
                ...
                <div
                    className={`flex justify-center mt-8 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"
                        }`}
